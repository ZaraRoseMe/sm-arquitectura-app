...
import { Plus, Search, FolderOpen, Calendar, CheckSquare, Trash2, Edit2 } from 'lucide-react'
...
export default function ProjectsClient({ projects: initialProjects, users, isAdmin }: ProjectsClientProps) {
            ...
            <div key={project.id} className="card p-5 group hover:shadow-md transition-all">
              ...
              {tasks.length > 0 && (
                <div className="mt-4 flex -space-x-2">
                  {Array.from(new Set(tasks.map((t: any) => t.user?.id)))
                    .slice(0, 4)
                    .map((uid: any) => {
