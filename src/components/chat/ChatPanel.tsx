...
export default function ChatPanel({ currentUserId, users, team: initialTeam, userRole }: ChatPanelProps) {
  ...
  // ─── Upload imagen via Uploadthing ────────────────────────────────────────
  async function handleImageUpload(winId: string, isGroup: boolean, file: File) {
    ...
    try {
      ...
      const res = await fetch('/api/uploadthing?actionType=upload&slug=chatImageUploader', {
        method: 'POST',
        body: formData,
      })
      ...
      if (isGroup && team) {
        ...
        await fetch('/api/messages', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content, receiverId: winId }) })
        fetchMessages(winId)
      }
    } catch (err) {
  ...
  function renderWindow(win: typeof chatWindows[0]) {
    ...
    const isUploading = uploading[win.id] || false
      ...
      <div key={win.id} className="flex flex-col bg-white dark:bg-neutral-900 rounded-t-2xl shadow-2xl border border-b-0 border-gray-100 dark:border-neutral-800"
        ...
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-t-2xl border-b border-gray-100 dark:border-neutral-800"
          ...
          {isGroup && !win.minimized && (
            <button onClick={e => { e.stopPropagation(); setShowGroupInfo(prev => ({ ...prev, [win.id]: !prev[win.id] })) }}
          ...
          <>
            {isGroup && showInfo && (
              <div className="border-b border-gray-100 dark:border-neutral-800 bg-gray-50/50 dark:bg-neutral-800/20 px-3 py-2.5 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {isCoordinador && (
                    <button onClick={() => { setNewGroupName(team?.name || ''); setEditingName(true); setShowGroupInfo(prev => ({ ...prev, [win.id]: false })) }}
            ...
            {isEmoji && (
              <div className="px-2 py-1.5 border-t border-gray-100 dark:border-neutral-800 flex flex-wrap gap-1">
                {EMOJIS.map(e => (
                  <button key={e} onClick={() => { setInputs(prev => ({ ...prev, [win.id]: (prev[win.id] || '') + e })); inputRefs.current[win.id]?.focus() }}
            ...
            <div className="px-2 py-2 border-t border-gray-100 dark:border-neutral-800 flex items-center gap-1.5">
              ...
              <input ref={el => { fileInputRefs.current[win.id] = el }} type="file" accept="image/*" className="hidden"
                onChange={e => { const file = e.target.files?.[0]; if (file) handleImageUpload(win.id, isGroup, file); e.target.value = '' }} />

              <input ref={el => { inputRefs.current[win.id] = el }}
                value={input}
