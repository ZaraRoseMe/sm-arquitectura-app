...
import { Send, MessageCircle, SmilePlus, ChevronDown, ChevronUp, Users, X, MoreHorizontal, Edit2, Check } from 'lucide-react'
...
export default function ChatPanel({ currentUserId, users, team: initialTeam, userRole }: ChatPanelProps) {
  ...
  const scrolledOnOpen = useRef<Record<string, boolean>>({})
        ...
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-t-2xl border-b border-gray-100 dark:border-neutral-800"
            ...
            <div className="flex-1 min-w-0">
              {isGroup && editingName && !win.minimized ? (
                <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                  <input
                    autoFocus
                    className="text-xs font-semibold bg-white/80 dark:bg-neutral-800/80 border border-brand-300 rounded px-1.5 py-0.5 outline-none w-full"
                    value={newGroupName}
                    onChange={e => setNewGroupName(e.target.value)}
                    onKeyDown={e => { if (e.key === 'Enter') saveGroupName(); if (e.key === 'Escape') setEditingName(false) }}
                  />
                  <button onClick={saveGroupName} className="flex-shrink-0 text-brand-600 hover:text-brand-700">
          ...
          {isGroup && !win.minimized && (
            <button
              ...
              className={cn('w-5 h-5 flex items-center justify-center rounded hover:bg-black/10 flex-shrink-0 transition-colors',
                showInfo ? 'text-brand-600' : 'text-gray-400')}>
            ...
            {isGroup && showInfo && (
              <div className="border-b border-gray-100 dark:border-neutral-800 bg-gray-50/50 dark:bg-neutral-800/20 px-3 py-2.5 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {isCoordinador && (
                    <button
                      onClick={() => { setNewGroupName(team?.name || ''); setEditingName(true); setShowGroupInfo(prev => ({ ...prev, [win.id]: false })) }}
                  ...
                  {groupMembers.map(m => (
                    <div key={m.id} className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-full flex items-center justify-center text-white flex-shrink-0"
                        style={{ backgroundColor: m.color || '#6366F1', fontSize: 8 }}>
